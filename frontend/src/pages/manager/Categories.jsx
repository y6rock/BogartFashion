  ...
  return (
    <div style={{ flex: 1, padding: '20px' }}>
      ...
      <div style={{ marginBottom: '30px', padding: '20px', border: '1px solid #333', borderRadius: '8px', boxShadow: '0 2px 4px rgba(0,0,0,0.15)', backgroundColor: '#181818' }}>
        <h3 style={{ marginTop: '0', marginBottom: '15px', color: '#C2883A' }}>{editingCategory ? 'Edit Category' : 'Add New Category'}</h3>
        <form onSubmit={editingCategory ? handleUpdateCategory : handleAddCategory} style={{ display: 'flex', gap: '10px' }}>
          <input
            ...
            style={{ flex: 1, padding: '10px', border: '1px solid #444', borderRadius: '5px', background: '#222', color: '#fff' }}
            required
          />
          ...
          <tbody>
            {categories.length === 0 ? (
              ...
              categories.map((category) => (
                <tr key={category.category_id} style={{ borderBottom: '1px solid #333' }}>
                  <td style={{ padding: '15px' }}>{category.category_id}</td>
                  <td style={{ padding: '15px' }}>{category.name}</td>
                  <td style={{ padding: '15px', textAlign: 'center' }}>
                    <button onClick={() => handleEditCategory(category)} style={{ padding: '8px 12px', backgroundColor: '#ffc107', color: '#222', border: 'none', borderRadius: '5px', cursor: 'pointer', marginRight: '10px' }}>Edit</button>
